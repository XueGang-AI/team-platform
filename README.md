...
- ✅ 本地 Web/API 已固定在 `3200/3201`；PostgreSQL/Redis 使用通用服务 `15432/16379`；Docker Compose 默认启动 OTel Collector、Prometheus、Loki、Tempo、Grafana 可观测性数据面骨架。
- ✅ 管理后台已完成真实性清理：项目目录只展示真实 API 项目，治理/发布/成本/观测无真实记录时显示真实空态，未接入控件禁用或说明原因，不用演示数据冒充真实状态。
- ✅ `manjv-studio` 已通过 `project.yaml` 接入测试；当前 canonical 环境为 `local` 与 `docker`，旧环境 slug 不应混入默认工作台。
...
> **端口说明**：PostgreSQL 使用通用服务 `127.0.0.1:15432/team_platform`，Redis 使用通用服务 `127.0.0.1:16379`，键前缀为 `team_platform:`。项目 compose 默认不再启动专属 PostgreSQL/Redis。
...
| 地址 | 用途 |
|------|------|
| `http://localhost:3200` | 团队项目治理平台总入口 |
| `http://localhost:3200/api/platform/*` | 管理后台同源代理到平台 API |
| `http://localhost:3200/api/platform/docs` | 真实 Swagger / OpenAPI 文档入口 |
| `http://localhost:3220` | Grafana 数据面入口；只有写入真实观测链接后才在平台观测页展示 |

> 说明：Web 与 API 仍是两个运行时进程，但浏览器侧统一从 `http://localhost:3200` 进入。`3201` 默认只作为本地 API 上游端口，由 Next.js `/api/platform/*` 代理承接页面请求；侧栏“平台 API 文档”打开 `/api/platform/docs`，不再渲染内部文档工作区。

如只需要启动基础设施：

```bash
pnpm dev:infra     # docker compose up -d（默认启动可观测性组件；PG/Redis 使用通用服务）
...
```bash
pnpm dev   # 同时启动 API(:3201) 与 Web(:3200)
```
...
```bash
TOKEN=$(node apps/cli/dist/index.js login --email admin@example.com --name Admin --api http://localhost:3201 | node -e 'let s="";process.stdin.on("data",d=>s+=d);process.stdin.on("end",()=>console.log(JSON.parse(s).token))')
node apps/cli/dist/index.js validate examples/project-manifests/manjv-studio.yaml --api http://localhost:3201
node apps/cli/dist/index.js apply examples/project-manifests/manjv-studio.yaml --api http://localhost:3201 --token "$TOKEN"
```
...
|------|------|
| `pnpm install` | 安装依赖 |
| `pnpm platform:start` / `pnpm platform:stop` | 启停本地总平台入口（默认 Web 3200、API 上游 3201） |
| `pnpm dev` | 启动 API + Web 开发模式 |
| `pnpm dev:infra` / `pnpm stop:infra` | 启停本地可观测性组件；PostgreSQL/Redis 使用通用服务 |
| `pnpm build` | 构建全部包（turbo） |
| `pnpm lint` | ESLint 检查 |
| `pnpm typecheck` | TypeScript 类型检查 |
| `pnpm test` | 单元测试 |
| `pnpm test:integration` | 集成测试（需通用 PostgreSQL/Redis 与必要环境变量） |
| `pnpm test:e2e` | Playwright E2E（当前 11 项，需先启动 infra + API + Web） |
...
浏览器侧通过 Web 代理访问 Swagger：`GET /api/platform/docs`。
